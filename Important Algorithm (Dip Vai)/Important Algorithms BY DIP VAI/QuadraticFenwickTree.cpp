...
//*************************************************My Code Starts Here*********************************************************************************
//it can be modified for quadratic function too
#define lim 101000
int dataMul[lim];
int dataMul2[lim];
int dataMul3[lim];
...
void internalUpdate(int at, int mul,int mul2,int mul3, int add) {
    while (at < lim) {
        dataMul[at] += mul;
        dataMul2[at]+= mul2;
        dataMul3[at]+= mul3;
...
void update(int left, int right, int by,int inc) {
    if(left>right) return;
    internalUpdate(left, by,inc,inc*left,-by*(left - 1) + inc*(left*left-(left*(left+1))/2));
    internalUpdate(right, -by,-inc,-inc*left, by * right+ inc*((right*(right+1))/2-right*left));
}
...
int query(int at) {
    ...
    //debug(start,mul,mul2,mul3);
    return ((start*(start+1))/2)*mul2 + (mul-mul3) * start + add;
}
