...
//nodes should be given in clockwise or anti-clockwise order
...
int InsidePolygon(Point* polygon, int N, Point p, int bound)
{
    ...
    {
        //point is an vertex
        if(EQ(p.x,p1.x)&&EQ(p.y,p1.y)) return bound;
        ...
        if(p.y > MIN(p1.y, p2.y) && p.y < MAX(p1.y, p2.y))
        {
            if(p.x>MAX(p1.x, p2.x)) continue;
            //x is before of ray
            else
            {
                //overlies on a horizontal ray
                if(EQ(p1.y,p2.y) && (!(p.x < MIN(p1.x, p2.x)))) return bound;

                //ray is vertical
                if(EQ(p1.x,p2.x))
                {
                    //overlies on a ray
                    if(EQ(p1.x,p.x)) return bound;
                    //before ray
                    else ++__count;
                }
                ...
                else
                {
                    ...
                    //overlies on a ray
                    if(EQ(p.x , xinters)) return bound;
