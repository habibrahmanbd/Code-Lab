...
//*************************************************My Code Starts Here*********************************************************************************
//complexity nk with no constant factor
#define s 50010
// k is the distance needed
VI adj[s];
LL ans;
LL child[s][4],parent[s][4];
int pare[s];
...
{
    parent[node][k%3]=parent[par][(k-1)%3];
    if(par) parent[node][k%3]+=child[par][(k-1)%3]-child[node][(k-2)%3];
    int i;
    child[node][k%3]=0;
}

void dfs3(int node,int par,int k)
{
    if(par) child[par][k%3]+=child[node][(k-1)%3];
}
...
LL getans(int n,int k)
{
    ...
    int i,j;
    for(i=1;i<=n;i++)
        child[i][1]=1;  //1 means less than 1
    for(i=2;i<=k+1;i++)
    {
        for(j=1;j<=n;j++)
            dfs2(j,pare[j],i);
    ...
    for(i=1;i<=n;i++)
        ans+=parent[i][(k+1)%3]+child[i][(k+1)%3];
    ans/=2;
}
