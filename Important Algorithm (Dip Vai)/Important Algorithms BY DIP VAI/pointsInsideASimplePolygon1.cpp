...
//1 based
bool pointinpoly(int node, float *x, float *y, float xx, float yy)  //nodes should be given in clockwise or anti-clockwise order
{
  int i, j;
  bool c=false;
  for (i = 1, j = node; i <= node; j = i++) {
    if ( ((y[i]>yy) != (y[j]>yy)) && //here all corner(vertex) case are handled
