...
//*************************************************My Code Starts Here*********************************************************************************
#define lim 1000100
//it is used in a range(sliding window)
//making it non-increasing (max value) or non-decreasing (min value) queue
LL arr[lim],C[lim],Id[lim];

//Returns true if l2 is better than line l1 when slope same
bool bad(int l1,int l2)
{
	return C[l1]>=C[l2]; // Min Value wanted... (reverse(< sign) for max value)
}

void add(long long c,int ind,int &last) //last=last index in stack
{
	...
	while (last>=2 && bad(last-2,last-1))
	{
	    C[last-2]=C[last-1];
	    Id[last-2]=Id[last-1];
	    last--;
...
long long query(int lowInd,int &pointer,int last) //lowInd=lowest legal index (starting index of sliding window)
{
