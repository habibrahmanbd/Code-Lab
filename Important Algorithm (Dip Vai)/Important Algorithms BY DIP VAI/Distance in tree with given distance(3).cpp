...
//*************************************************My Code Starts Here*********************************************************************************
// complexity nlogk with less significant constant factor(very fast 92 ms in codeforces)
// when there is only a few k then this should be used
#define s 50010
// k is the distance needed
VI adj[s];
LL ans;

void dfs(int node,int par,int k,vector<int> &a)
{
    //print1(node);
    int i,j;
    a.pb(1);
    for(i=0;i<SZ(adj[node]);i++)
    {
        ...
        dfs(tem,node,k,b);
        b.pb(0);
        // merging smaller tree into larger tree
        if(SZ(a)<SZ(b))
            a.swap(b);
        //print1(node);
        //debug_array(a,SZ(a));
        //debug_array(b,SZ(b));
        for(j=max(0,SZ(b)-k-1);j<SZ(b)&&SZ(a)+SZ(b)-k-j-2>=0;j++)
            ans+=b[j]*a[SZ(a)+SZ(b)-k-j-2];
        for(j=SZ(b)-1;j>=0;j--)
            a[SZ(a)-SZ(b)+j]+=b[j];
    }
}
