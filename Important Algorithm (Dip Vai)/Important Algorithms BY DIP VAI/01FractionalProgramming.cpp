...
bool chc(double val) //main 0-1 Fractional-Programming is done here
{
    int i;
    for(i=1;i<=n;i++)
        d[i]=100.0*(double)a[i]-val*(double)b[i];
    sort(&d[1],&d[n+1]);
    double ret=0;
    for(i=k+1;i<=n;i++)
        ret+=d[i];
...
double bs(double st,double end)
{
    double mid=(st+end)/2;
    //print2(st,end);
    if(end-st<ERR) return mid;
    if(chc(mid)) return bs(mid,end);
    return bs(st,mid);
}
...
int main()
{
    ...
    while(cin>>n>>k&&(n||k))
    {
        ...
        for(i=1;i<=n;i++)
            cin>>b[i];
        double ret=bs(0,1000000000000.0);
        //printf("%lf\n",ret);
        LL ans;
        round(ans,ret);
        print1(ans);
