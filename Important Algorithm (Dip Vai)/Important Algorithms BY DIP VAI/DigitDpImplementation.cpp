...
int dprec(int len,int mod,int remsum,int tar)
{
    if(len<0)
    {
        if(mod==0 && remsum==0) return 1;
        return 0;
    }
    int &ret = dp[len][mod][remsum][tar];
    if(ret!=-1) return ret;
    ret=0;
    for(int i=0;i<=9;i++)
    {
       if(i>remsum) break;
       ret+=dprec(len-1,(10*mod+i)%tar,remsum-i,tar);
    }
    return ret;
}
int arr[20];

int rec(int len,int mod,int remsum,int tar)
{
    //debug(len,mod,remsum,tar);
    if(len<0)
    {
        if(mod==0 && remsum==0) return 1;
        return 0;
    ...
    for(int i=0;i<arr[len];i++)
    {
        if(i>remsum) break;
        ret+=dprec(len-1,(10*mod+i)%tar,remsum-i,tar);
        //debug(len,mod,remsum,tar,ret);
    }
    if(arr[len]<=remsum) ret+=rec(len-1,(10*mod+arr[len])%tar,remsum-arr[len],tar);
    //debug(len,mod,remsum,tar,ret);
    return ret;
}
