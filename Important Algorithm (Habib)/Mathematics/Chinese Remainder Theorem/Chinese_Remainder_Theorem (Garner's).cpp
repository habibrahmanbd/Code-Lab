...
//*************************************************My Code Starts Here*********************************************************************************
//a=x0+x1*p0+x2*p0*p1+x3*p0*p1*p2+....+x(k-1)*p0*p1*p2**..p(k-2) (mod p0*p1*p2*...p(k-1))


void chineseremaindertheorem(LL x[],LL a[],LL r[][100],LL p[],LL k) //a=remainder, r[j][i]=p[j]^-1 (mod p[i]), p=primes (0 based)
{
    for ( LL i = 0 ; i < k ; ++ i ) {
        x [ i ] = a [ i ] ;
	for ( LL j = 0 ; j < i ; ++ j ) {
