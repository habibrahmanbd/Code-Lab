...
//O based
//heap[x][y]=a[x]+a[x+1]+.........+a[x+2^y-1] (x should be divisible by 2^y)
#define lim 100010
#define loglim 17 //ceiling(have to be precise because of memory limit)
int heap[3*lim][loglim+1]; //first dimension can be flexible
//note that the value of IDENTITY should be such that f(IDENTITY, x) = x, e.g. 0 for sum,  + ∞ for min,  - ∞ for max, and 0 for gcd.


void initialize() {
  for (int x = 0; x < lim; x++)
    heap[x][0] = a[x]; //intial array
  for (int y = 1; y <= loglim; y++)
    for (int x = 0; x < lim ; x+=(1<<y)) //x should be divisible by 2^y
      heap[x][y] = f(heap[x][y-1], heap[x+(1<<(y-1))][y-1]);
}
...
void update(int x, int v) {
  ...
  for (int y = 1; y <= loglim; y++) {
    int xx = x-(x&((1<<y)-1));
    heap[xx][y] = f(heap[xx][y-1], heap[xx+(1<<(y-1))][y-1]);
  }
}

int query(int i, int j) {
  int res = IDENTITY, h = 0; j++; //identity means the starting value
  while (i+(1<<h) <= j) {
