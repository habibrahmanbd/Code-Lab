...
string str1, str2;
ll len1, len2; //length of string.
ll dp[1002][1002]; //stores the maximum length of LCS
ll pr[1002][1002]; //1 for up(|), 2 for left(-) and 3 for upper corner(\)

//The Complexity for LCS_Length is O(Len1*Len2)

void LCS_Length() //id1 & id2 res. pointing the str1, str2
{
    for(ll id1=1; id1<=len1; id1++)
    {
        for(ll id2=1; id2<=len2; id2++)
        {
            ...
            {
                dp[id1][id2]=dp[id1-1][id2];
                pr[id1][id2]=1;
            }
            else
            {
                dp[id1][id2]=dp[id1][id2-1];
                pr[id1][id2]=2;
            }
        }
    }
...
//The Complexity for PRINT_LCS is O(Len1+Len2)

void PRINT_LCS(ll id1 ,ll id2)
{
    if(id1==0||id2==0)
        return;
    if(pr[id1][id2]==3)
    {
        PRINT_LCS(id1-1, id2-1);
        pf("%c",str1[id1-1]);
    }
    else if(pr[id1][id2]==1)
    {
        PRINT_LCS(id1-1, id2);
    }
    else
...
int main()
{
    ...
    len1 = str1.length();
    len2 = str2.length();
    clr(dp,0); //initially length is zero
    clr(pr,-1); //initially print position
    LCS_Length(); //Finds the LCS Length
    PRINT_LCS(len1, len2); //prints the LCS
    for(ll i=1; i<=len1; i++)
    {
        cout<<endl;
        for(ll j=1; j<=len2; j++)
